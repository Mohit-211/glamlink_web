...
export default function MagazineIssues() {
  ...
  // ✅ ONLY for 2026 (latest first by id)
  const sort2026Issues = (issues: Issue[]) => {
    ...
    <section className="py-16 lg:py-24 bg-gradient-to-b from-background to-secondary/5">
      <div className="container-glamlink max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {sections.map(({ year, issues }) => (
          <div key={year} className="mb-16 lg:mb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {(year === 2026
                ? sort2026Issues(issues) // 🔥 only 2026 sorted
                : issues
              ).map((issue) => (
        ...
        <div className="mt-12 lg:mt-16 text-center">
          <Button
            variant="outline"
            className="rounded-full px-8 py-6 text-base border-2 border-[#24bbcb]/50 hover:border-[#24bbcb] hover:bg-[#24bbcb]/5 hover:text-[#24bbcb] transition-all duration-300 group"
          >
            Browse Full Archive
