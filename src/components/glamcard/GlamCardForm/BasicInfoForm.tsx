...
const BasicInfoForm: React.FC<Props> = ({ data, setData }) => {
	...
	const autocompleteRefs = useRef<
		Record<string, google.maps.places.Autocomplete>
	>({});
	...
	useEffect(() => {
		...
		data.locations.forEach((loc) => {
			const input = addressRefs.current[loc.id];
			if (!input || autocompleteRefs.current[loc.id]) return;
			...
			autocompleteRefs.current[loc.id] = autocomplete;
		});
	}, [data.locations]);
							...
							<div className="space-y-5 p-5">
								...
								<div className="flex gap-6">
									<label>
										<input
											...
											onChange={() => updateLocation(loc.id, { type: "exact" })}
										/>{" "}
										Exact Address
									</label>
									...
									<label>
										<input
											...
											onChange={() => updateLocation(loc.id, { type: "city" })}
										/>{" "}
										City Only
									</label>
								</div>
								...
								{loc.type === "exact" && (
									<>
										<div className="flex gap-3">
											<input
												ref={(el) => (addressRefs.current[loc.id] = el)}
												className={inputClass}
												value={loc.address}
												onChange={(e) =>
                        ...
                        Use Address
										...
										{loc.isSet && (
											<>
												...
												{loc.isSet && loc.lat && loc.lng && (
													<LocationMap lat={loc.lat} lng={loc.lng} />
												)}
											</>
										)}
									</>
